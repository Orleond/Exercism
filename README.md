...
### Список задач:
 ...
 - [x] [Задача 17. Тим из маркетинга](#task17)
...
### <a name="task13">Задача 13. Калькулятор зарплаты</a>
...
Каждая строка журнала представляет собой строку, отформатированную следующим образом: `"[<LVL>]: <MESSAGE>"`.
...
Закодированный уровень журнала представляет собой простое сопоставление уровня журнала с числом:
...
### <a name="task17">Задача 17. Тим из маркетинга</a>
###### [Решение](https://github.com/Orleond/Exercism/tree/main/src/main/java/TimFromMarketing)
В этом упражнении вы напишете код для печати бейджей с именами сотрудников завода.
<br>

___
#### ___Задание 1. Распечатать бейдж для сотрудника___
У сотрудников есть идентификатор, имя и название отдела. Этикетки на бейджиках сотрудников имеют следующий формат: `"[id] - name - DEPARTMENT"`. Напишите реализацию метода `Badge.print()` для возврата содержимого бейджа сотрудника:
``` Java
...
___
#### ___Задание 2. Распечатать бейдж для нового сотрудника___
Из-за особенностей компьютерной системы новые сотрудники иногда еще не имеют удостоверения личности, когда начинают работать на фабрике. Поскольку бейджи обязательны, они получат временный бейдж без префикса идентификатора. Измените метод `Badge.print()` для поддержки новых сотрудников, у которых еще нет идентификатора:
``` Java
Badge badge = new Badge();
...
___
#### ___Задание 3. Распечатать бейдж владельца___
Даже владелец фабрики должен постоянно носить бейдж. Однако у владельца нет отдела. В этом случае вместо названия отдела должна печататься надпись `"OWNER"`. Измените метод `Badge.print()` для печати бейджа владельца:
``` Java
Badge badge = new Badge();
badge.print(254, "Charlotte Hale", null);
// => "[254] - Charlotte Hale - OWNER"
```
Обратите внимание, что владельцем может быть и новый сотрудник:
``` Java
Badge badge = new Badge();
